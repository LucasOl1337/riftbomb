...
| **Playback in game** | **Capped to blast visual life (`0.5` s)** in `play-rift-sfx.js` via `visualLife` / envelope — file length ≠ audible length |
...
- Keep sample playback ≤ `blasts[].life` (today `0.5`). Changing blast life requires changing audio window.
