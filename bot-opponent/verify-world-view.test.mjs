...
function makePlayer(id) {
  return {
    id,
    champion: id === 1 ? "katarina" : "zed",
    side: id === 1 ? "blue" : "red",
    name: id === 1 ? "P1" : "Red Zed",
    x: id === 1 ? -7.26 : 7.26,
    z: id === 1 ? 5.94 : -5.94,
...
test("buildWorldView returns P0 WorldView for the default bot id 2", () => {
  ...
  assert.equal(view.self.champion, "zed");
...
test("buildWorldView never freezes live blast or pickup objects", () => {
  const match = stubMatch();
  const blast = match.blasts[0];
  const pickup = match.pickups[0];
  const view = buildWorldView(match, 0.016);

  assert.notEqual(view.blasts[0], blast);
  assert.notEqual(view.pickups[0], pickup);
  assert.equal(Object.isFrozen(blast), false);
  assert.equal(Object.isFrozen(pickup), false);
  assert.equal(Object.isFrozen(view.blasts[0]), true);
  assert.equal(Object.isFrozen(view.pickups[0]), true);
});
