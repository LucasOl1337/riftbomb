...
if (!inputPath || !outputDirectory) {
  throw new Error("Usage: node bake-zed.mjs <animated.glb> <output-directory>");
}
...
if (!meshes.length) throw new Error("No playable Zed skinned meshes found");
...
const unitScale = 2.05 / sourceHeight;
...
const normalIdle = sampleSmoothNormals("zed_idle1", 0.0);
const normalSkill = sampleSmoothNormals("zed_spell4_leadin", 0.72);
...
await Promise.all([
  fs.writeFile(path.join(outputDirectory, "zed-model-vertices.bin"), Buffer.from(vertices.buffer)),
  fs.writeFile(path.join(outputDirectory, "zed-model-indices.bin"), Buffer.from(indices.buffer)),
  fs.writeFile(path.join(outputDirectory, "zed-model-metadata.json"), JSON.stringify({
    source: "Classic Zed game mesh and matching base rig",
    unitScale,
    vertexCount,
    indexCount,
    strideFloats,
