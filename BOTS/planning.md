...
│                                                          │
│   Match state ──► buildSnapshot() ──► BOTS política      │
│                                      │                   │
...
2. Criar `BOTS/rift-cpu-policy.js` (nome final a decidir) com a lógica atual de `updateBot`.
3. Em `Game.update`, se `!p2Human`, chamar política e copiar `aiDx`/`aiDz` + `placeBomb` / skills.
4. Deixar `updateBot` como thin wrapper até o build (`assemble-riftbomb.mjs`) incluir o script de BOTS na ordem certa — **ou** manter o arquivo de política importado só no assemble.

### Build / HTML autocontido

Hoje o assemble empacota scripts listados em `game/play-riftbomb.html`.  
Quando houver código em `BOTS/`:

- ou o HTML de entrada referencia `../BOTS/...` e o assembler resolve
- ou `game/` reexporta um único bridge fino

Decisão preferida: **bridge fino em `game/`** que só instancia a política; corpo da política em `BOTS/`. Assim a geografia do produto continua legível (“regras em game, cérebro do oponente em BOTS”).

### Geografia do repo

- Profundidade máxima 3 níveis (gate existente).
- Evitar pastas folha com um único arquivo.
- Nome `BOTS` é de domínio (oponentes), não camada técnica proibida (`utils`, `core`, …).
...
3. Portar `updateBot` linha a linha para `BOTS/`.
...
Registrar notas em `BOTS/` só quando uma sessão de playtest mudar requisito — não log diário obrigatório.
